...
public class SafeFrame extends Frame implements ActionListener, Context {
    ...
    public SafeFrame(String title) {
        ...
        textClock.setEditable(false); // 위치 이동이 안되도록 막아놓음(편집 방지?)
        // textScreen 배치 
        add(textScreen, BorderLayout.CENTER);
        textScreen.setEditable(false); // 위치 이동이 안되도록 막아놓음(편집 방지?)
        // 패널에 버튼 저장
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == buttonUse) {		// 금고 사용 버튼
            state.doUse(this); // 현재 객체에서 위임(제일 중요한 부분!!*******)
    ...
    public void changeState(State state) {
        System.out.println(this.state + "에서" + state + "으로 상태가 변화했습니다."); // 상태 바꾸기 전 바꾸는 상태에 대한 안내
        this.state = state; // 상태([주간] or [야간])를 바꿈
    }
